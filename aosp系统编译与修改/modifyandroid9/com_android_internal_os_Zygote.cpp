...
//add
void FilterLog(const char *thread_name) {
    FILE *file_filter = fopen("/data/local/tmp/thread_filter.txt", "r");
    if (file_filter == NULL) {
        ALOGW("Failed to open /data/local/tmp/thread_filter.txt.");
        return;
    }
    char buffer[40] = {0};
    fread(buffer, 40, 1, file_filter);
    if (strlen(buffer) == 0) {
        return;
    }

    char thread_buf[40] = {0};
    strncpy(thread_buf, thread_name, strlen(buffer));
    if (strncmp(buffer, thread_buf, strlen(buffer)-1) == 0) {
        android::base::SetMlogFilter(true);
    }
    ALOGW("filter log thread name %s, filter name %s, filter flag %s", thread_name, buffer,
          (android::base::GetMlogFilter() ? "true" : "false"));
    fclose(file_filter);
}
