...
void InvokeWithArgArray(const ScopedObjectAccessAlreadyRunnable& soa,
  ...
  if(android::base::GetMlogFilter()){
    ...
    if(managedStack!=nullptr){
      ...
      if(tmpArtmethod!=nullptr){
        ...
        LOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << " caller:"<< artmethod->PrettyMethod() << ", called:" << method->PrettyMethod();
      }
    }
  }
  ...
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return object";
  //     break;
  //   case Primitive::kPrimBoolean:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return boolean:" << (result->GetZ() == JNI_TRUE ? "true" : "false");
  //     break;
  //   case Primitive::kPrimByte:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return byte:" << result->GetB();
  //     break;
  //   case Primitive::kPrimChar:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return char:" << result->GetC();
  //     break;
  //   case Primitive::kPrimShort:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return short:" << result->GetS();
  //     break;
  //   case Primitive::kPrimInt:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return int:" << result->GetI();
  //     break;
  //   case Primitive::kPrimLong:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return long:" << result->GetJ();
  //     break;
  //   case Primitive::kPrimFloat:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return float:" << result->GetF();
  //     break;
  //   case Primitive::kPrimDouble:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return double:" << result->GetD();
  //     break;
  //   case Primitive::kPrimVoid:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return void";
  //     break;
  //   // case Primitive::kPrimLast:
  //   //   LOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << ", return void";
  //   //   break;
  //   default:
  //     MLOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod() << "Unexpected result";
  // }
  // LOG(ERROR) << "InvokeWithArgArray uid:" << getuid() << ", method name:" << method->PrettyMethod();
  ...
  //   uint16_t tries_size = accessor.TriesSize();
  //   if (tries_size>0) {
  //     // const uint8_t *handler_data make= (const uint8_t *)(DexFile::GetTryItems(*code_item, code_item->tries_size_));
  //     const uint8_t *tries = (const uint8_t *)accessor.TryItems().begin();
  //     // uint32_t start = tries->start_addr_;
  //     // const uint8_t *handler_data = (const uint8_t *)(DexFile::GetTryItems(accessor.begin(), tries_size));
  //     uint8_t * tail = codeitem_end(&tries);
  //     code_item_len = (int)(tail - item);
  //   }else{
  //     uint32_t insns_size_in_code_units = accessor.InsnsSizeInCodeUnits();
  //     code_item_len = 16 + insns_size_in_code_units * 2; // 16是code_item前16个字节，insns_size_in_code_units * 2是insns
  //   }

  //   char tmp[200] = {0};
  //   // char* base64result=base64_encode((char*)item,(long)code_item_len,&outlen);
  //   memcpy(tmp, (char*)item, (long)code_item_len);
  //   LOG(ERROR) << "InvokeWithArgArray method name " << method->PrettyMethod() << " params size " << arg_array->GetNumBytes() << " inins " << tmp;
  // }
}
